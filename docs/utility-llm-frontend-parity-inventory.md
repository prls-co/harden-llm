...
| `Web search` / visible `🌐` | Pressed toggle button | Turns web search on or off for the next run. The gateway uses native Responses search when the selected profile advertises it and Jina otherwise; normal cache lookup and refresh apply to search runs too. Accessible label, pressed state, and title describe the current mode. |
...
| `Structured output` | Select (`Off` / `On`) | Explicitly enables or disables structured output inside Advanced input. The schema draft is retained when Off; On requires a valid schema. |
| `Structured Output Schema` | Monospace multiline textarea | Full JSON Schema object. It is checked explicitly and after five seconds of inactivity; invalid, pending, or failed checks block a structured run when enabled. |
...
- `ProfileDefaults` is the single frontend source for the utility-aligned editor defaults: `max_tokens: 16000`, temperature/top-p/top-k/stop-sequence placeholders, retry and escalation defaults (`CPA GPT-5.6 Sol` / `gpt-5.6-sol`), model/base-URL/profile placeholders, reasoning/cache defaults, and contextual `?` help text. The shared input component renders each `?` as a keyboard-accessible button that toggles an inline explanation, starts collapsed, and stays outside the field label so opening help cannot activate its input. `WEB-TEST-052` covers the pure default contract, rendered help text, field associations, and serialized client disclosure commands; it does not establish browser execution or layout.
...
- The workspace input widget now follows utility-llm's prompt/advanced/schema topology: utility placeholders and row counts, monospace schema fields, right-aligned action rows, a field-local schema hint, conditional advanced rendering, populated default prompt/schema values, and the contracted schema keyword/enum guard. A single Advanced input selector explicitly enables or disables structured output; the API `callType` remains the required persisted/run boundary, and a schema draft is retained without blocking text runs when structured output is Off. Structured Repair is exposed once in the profile `Retries & Repair` fold. `WEB-TEST-056`, `WEB-TEST-057`, and `WEB-TEST-059` cover these server-owned defaults, rendered diffs, validation, Run gating, explicit mode switching, and profile repair projection; the `SchemaCheck` and `SchemaPending` hooks have only their pure client decisions covered by Node, so no browser permutation is added for backend invariants.
...
- The input-widget follow-up is also covered at the cheaper LiveView tier:
  `WEB-TEST-056`, `WEB-TEST-057`, and `WEB-TEST-059` assert utility input
  topology/defaults, contracted schema validation, invalid-schema Run gating,
  explicit response-mode switching, and profile repair projection. Node covers the
