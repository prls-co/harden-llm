...
| `Structured Output Schema` | Monospace multiline textarea | Full JSON Schema object. It is checked explicitly and after five seconds of inactivity; an empty schema selects a text run, while invalid, pending, or failed checks block the inferred structured run. |
...
| `Run Prompt` | Primary button | Runs the selected profile with the current prompts, schema-derived response mode, reasoning, cache mode, default options, and retry/repair policy, then addresses the conversation at `/workspace?trace_id=...`. Disabled without a profile, while running, or while schema validation blocks execution. |
...
- The workspace input widget now follows utility-llm's prompt/advanced/schema topology: utility placeholders and row counts, monospace schema fields, right-aligned action rows, a field-local schema hint, conditional advanced rendering, populated default prompt/schema values, and the contracted schema keyword/enum guard. Schema presence is the response-mode switch: an empty schema produces a text request and a valid schema produces a structured request; the API `callType` remains a required derived boundary rather than a duplicate input control. Structured Repair is exposed once in the profile `Retries & Repair` fold. `WEB-TEST-056`, `WEB-TEST-057`, and `WEB-TEST-059` cover these server-owned defaults, rendered diffs, validation, Run gating, and profile repair projection; the `SchemaCheck` and `SchemaPending` hooks have only their pure client decisions covered by Node, so no browser permutation is added for backend invariants.
...
- The input-widget follow-up is also covered at the cheaper LiveView tier:
  ...
  schema-derived response mode, and profile repair projection. Node covers the
