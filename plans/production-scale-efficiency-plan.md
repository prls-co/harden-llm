...
- `P01.S07 Exercise owned cancellation against a real Docker daemon`
  - Progress: DONE — deterministic policy and fast gates pass. Real Docker TEST-274 passed four scenarios (success, partial create, TERM, supervisor SIGKILL/next-owner recovery), preserved the independently receipted sentinel, left all receipts `cleaned`, and produced no cleanup warnings/errors. Exact-SHA hosted release [35718249789](https://github.com/prls-co/harden-llm/actions/runs/35718249789) then passed the unchanged full Compose smoke on the retry path (`go-compose` 238.340 s), including bootstrap/login/run/correlation assertions, with no timeout or cleanup warning/error. The earlier 321.765 s failure remains recorded below as a corrected failed attempt; no test timeout or assertion was changed.
...
- `P04.S05 Certify and publish the exact main source`
  - Progress: DONE — source `6887fcd8146961dc64598dd7a236e7a9fc522c9c` is on `origin/main`. Main fast workflow [35726127323](https://github.com/prls-co/harden-llm/actions/runs/35726127323) and CodeQL [35726126610](https://github.com/prls-co/harden-llm/actions/runs/35726126610) succeeded. Publisher [35726391122](https://github.com/prls-co/harden-llm/actions/runs/35726391122) passed exact-source `make test-release`: 28 tasks, `accepted=true`, no task failure, timeout, cleanup error, or cleanup warning. GHCR package visibility is `private`; tag `6887fcd8146961dc64598dd7a236e7a9fc522c9c-35726391122-1` resolves to `ghcr.io/prls-co/harden-llm-gateway@sha256:036d82749a1e5a29e36c848d5fca955c4b416858c9ea272f2cf1b4428210905b`. Registry manifest and pulled image are `linux/amd64`; OCI source/revision/version and attached BuildKit provenance identify the same repository and full source SHA.
...
- `P04.S06 Deploy the published image by digest and verify the gateway boundary`
  - Progress: DONE — pulled and deployed the exact private digest above. Production gateway container `7d8b8bb25e0576c20a3cb81c14d86fe90eedeffa1b8621a4b6ea2841cc03c457` is running/healthy with zero restarts; configured digest, image ID, OCI source/revision/version, and `HARDEN_LLM_RELEASE` match source `6887fcd8146961dc64598dd7a236e7a9fc522c9c`. Gateway-only `production-config apply` and the final gateway+web check passed. Web container/image stayed unchanged and healthy. Public API `/healthz` and `/readyz`, web `/healthz` and `/login` each returned HTTP 200. Read-only `audit-artifacts` reported 5/5 objects referenced, zero active operations, zero missing/unreferenced objects, `truncated=false`, `healthy=true`. The prior descriptor is retained mode 600 at `/home/kirill/.config/harden-llm/rollout-6887fcd.ocJxGl/production.before.json` (its parent directory is mode 700); only the four gateway image/release identity fields differ from the deployed descriptor.
...
- `P04.S07 Close lifecycle records and operator follow-ups`
  - Progress: DONE — exact source, certification, package visibility/digest/provenance, scoped production promotion, retained rollback identity, test exclusions, and follow-up risks are recorded in this plan and `docs/release-certification.md`. P03 capacity holdout remains conditional/unrun; browser and live-provider checks remain unverified by policy.
...
- Bootstrap: P02.S01.
- Fixtures/mocks/data: Synthetic byte/token/price datasets, mismatched fingerprints, unknown actual CPA price, missing SLO, bounded-host resource and exporter-drop cases, a four-case/2,000-artifact-per-case exploration report.
- Deterministic controls: Fixed decimal inputs and seed 104729; no external pricing lookup or real calls.
- Pass criteria: Exact denominator/unit arithmetic; incompatible samples not compared; missing prices/metrics are null with reason; p99 is absent below 1,000 samples; SSE rates/volume are summarized; stage/model token totals aggregate exactly; maximum supported report publishes under 1 MiB with no raw request/artifact arrays, at most 96 traceable request diagnostics and 96 artifact metadata samples per scenario, exact artifact counts/bytes, and explicit omitted counts; no unsupported savings/SLO claim; only four allowed dispositions.
- Expected runtime: 5 seconds.
...
This section is the implementation ledger. Planning validation is not implementation completion. The YAML entry below is reusable; keep failures and superseded measurements visible.
...
| P04 | Done; private publication and gateway-only production deployment accepted | Exact source `6887fcd8146961dc64598dd7a236e7a9fc522c9c` is on `origin/main`; hosted T0-T2, CodeQL, and browser-free 28-task release checks passed; the private GHCR digest and BuildKit source revision were verified; only the gateway was changed and its production config, health, public probes, and artifact inventory passed. See the P04 closeout below. Capacity remains `insufficient_evidence`. |
...
- P03 amendment `P03.A02`: this implementation request explicitly authorizes production completion of P03.A01. The changed service is only `harden-llm-gateway`; the root `Dockerfile` builds the immutable release from source SHA in the existing production Compose checkout. The existing production descriptor points to a never-reused `harden-llm-gateway:release-<full-sha>` local image and `scripts/production-config.mjs apply --services harden-llm-gateway --expected-release <full-sha>` performs the scoped no-build/no-deps update. Capture the exact local Docker image ID; there is no configured registry workflow/target in this repository, so local immutable-image publication is the applicable artifact handoff, not a claim of registry publication. Keep the web and all data/session volumes unchanged; no migration, config/profile sync, or provider call. Roll back only the gateway to its captured previous exact image through the same scoped descriptor/apply path. Verify source release, container health/restarts, gateway readiness, frontend/API HTTP health, and read-only artifact integrity/backlog; browser and paid-provider tests remain unrun.
...
- Corrected hosted integration run [35713387932](https://github.com/prls-co/harden-llm/actions/runs/35713387932) on `91094fb5e67f4a4836ea087f557e4a6c57311db1` accepted both `go-integration` (41.666 s) and `go-integration-race` (112.633 s), including TEST-060, with no timeout, cleanup error, or cleanup warning. TEST-060 GREEN is now established for the hosted PostgreSQL/Garage boundary; proceed to exact-SHA TEST-277 correctness, then exploration.
...
- At the pre-promotion `aea1e42` checkpoint, seven commits were ahead of the unchanged `origin/main` base. The final tested release candidate `d85f7db` added the execution-evidence documentation commit (eight commits ahead in total), was reviewed, and was pushed to `main` at the exact tested SHA.
...
- At candidate `aea1e42`, pinned `make test-fast` passed with `accepted:true`, all 10 tasks, no failure, timeout, cleanup error, or cleanup warning. Retained local report: `tmp/test-feedback/runner-1790074226522-3851758-cb069f0c797005e4.json`. The later exact-SHA release, main promotion, gateway image/deployment, and artifact inventory are recorded in the dated P01/P04 closeout below.

### P01/P04 exact-source release and gateway deployment — 2026-09-22

- Candidate/source: `d85f7dbf3be7dccf2f5c8f21fd161d4bc91631ec`; feature branch `codex/artifact-publication-grace`; pushed to `origin/main`, whose verified ref equals this SHA. It is eight commits ahead of the prior main base `81e9d1d59c4c1f264eb3f851eeaf550a42c38e69`; review found only the artifact-publication grace/report-bound implementation and corresponding tests, catalog, ADR, and plan updates. The deployed gateway source release `12b0478af9ef59c36a08011c5556d8e37ff9625c` is an ancestor of the candidate, and the candidate adds no Postgres migration files.
- Local pinned `make test-fast`: accepted all 10 tasks, no failures/timeouts/cleanup errors/warnings; report `tmp/test-feedback/runner-1790074226522-3851758-cb069f0c797005e4.json`. Hosted exact-SHA browser-free release [35718249789](https://github.com/prls-co/harden-llm/actions/runs/35718249789): all 28 tasks accepted in 15m26s, no task timed out, no cleanup errors/warnings. `go-integration` 82.306 s, `go-integration-race` 205.498 s, `go-compose` 238.340 s, and `go-race` 250.306 s. The unchanged Compose smoke/bootstrap/login/run/correlation assertions passed. Downloaded redacted runner reports are under `tmp/test-feedback/gh-release-d85f7db/`; full release report is `harden-llm-runner-reports-35718249789-1-release/runner-1790075208906-2636-6572bba6801ccbdc.json`. Automatic main-branch fast workflow [35719965824](https://github.com/prls-co/harden-llm/actions/runs/35719965824) also passed in 1m45s. Tier policy and `git diff --check` passed before promotion.
- Browser, browser-compose, explicit lifecycle-fault, and synthetic-capacity jobs were skipped by the selected browser-free release workflow. No browser or real-provider call was run. P02 correctness/exploration evidence remains the exact-source hosted reports on `2380824dfd7b693df58f643d5b81019fe25e2edd`; the later commits changed documentation only. Both reports retain `insufficient_evidence`, not capacity/cost certification; EVAL-011 holdout remains unrun without a production SLO and approved operating point.
- Production preflight: `production-config check` was equivalent for the old gateway; source, image, and running container agreed on prior release `12b0478...`, old image `sha256:9395f2465e32a5f3b00802e5eff24ad7d98f5d717e63acd0a0aeba84f6618673`, healthy state, and zero restarts. Public API `/healthz` and `/readyz`, web `/healthz` and `/login` all returned HTTP 200 before and after deployment. The production Docker context is `default` on host `shaman` (12 CPUs, 32.75 GB RAM, 109 running containers). Before the single-service build, available RAM was about 6.2 GB, swap was nearly full, but memory PSI was near zero; three `vmstat` samples showed 70–85% CPU idle and 1–2% I/O wait. Docker-root filesystem free space was 291,492,528,128 bytes. A read-only `docker system df` query did not return within 60 seconds and was terminated to limit additional host load, so exact Docker image/cache/volume allocation was not measured. The build used the existing Dockerfile and cached Go layers; it took about 20 seconds with a 581.67 KB context and did not restart services. These are point-in-time safety observations, not capacity evidence or a cold-build performance claim.
- Built local immutable tag `harden-llm-gateway:release-d85f7dbf3be7dccf2f5c8f21fd161d4bc91631ec`; Docker image ID `sha256:ac30bb305f9cd4711c15acf35eb6fb57b0c91c2e6d21d23a7c27ede403a864e9`; OCI version label and isolated `version` invocation both match the full source SHA. No registry workflow/target exists, so no remote image digest or registry publication is claimed. Source and hosted CI evidence are published on GitHub `main`; the runtime image is local to the existing production Docker host.
- Private rollback checkpoint: `/home/kirill/.config/harden-llm/rollout-d85f7db.4IFr2w/production.json.before` (directory mode 700, file mode 600; SHA-256 `47572a6faf372c156000bbc47bdbc375f32ba568f55d58acc710e34a7d9361ab`). The previous gateway image/tag remains present locally for rollback (`sha256:9395f2465e32a5f3b00802e5eff24ad7d98f5d717e63acd0a0aeba84f6618673`, `release-12b0478`). Only the gateway expected image, image override, and its two release identity fields changed in the private descriptor; no secrets or descriptor contents were committed.
...
- This supersedes the earlier no-registry-target statement in the historical d85 deployment entry: that statement accurately described the state at that checkpoint. Candidate `6887fcd8146961dc64598dd7a236e7a9fc522c9c` was pushed to and independently verified on `origin/main` and the feature branch. Exact-source hosted fast CI, CodeQL, all 28 browser-free release tasks, private package visibility, digest, platform, labels, and attached BuildKit provenance were read back successfully. Publisher report artifact: `harden-llm-gateway-6887fcd8146961dc64598dd7a236e7a9fc522c9c-35726391122-1`. The immutable production reference is `ghcr.io/prls-co/harden-llm-gateway@sha256:036d82749a1e5a29e36c848d5fca955c4b416858c9ea272f2cf1b4428210905b`; no public tag or fallback was used.
- Gateway-only production promotion completed on host `shaman` through Docker context `default`. The preflight descriptor was equivalent at old release `d85f7dbf3be7dccf2f5c8f21fd161d4bc91631ec`; its mode-600 backup is `/home/kirill/.config/harden-llm/rollout-6887fcd.ocJxGl/production.before.json`, SHA-256 `16b9d8b99115eb195c40dc741d7aec095cec1706d1ebc9c387ceb3b369a6b780`. The replacement descriptor is also mode 600, SHA-256 `978deb24665dfa227caf4bcda443f0a4b63ab2f0330f7db0e46f72f34fb18d4a`; a normalized comparison proved that only gateway expected image, image reference, identity release, and service release override changed. Previous local image `harden-llm-gateway:release-d85f7dbf3be7dccf2f5c8f21fd161d4bc91631ec` (`sha256:ac30bb305f9cd4711c15acf35eb6fb57b0c91c2e6d21d23a7c27ede403a864e9`) remains available for gateway rollback.
- Post-deploy: the gateway is healthy with zero restarts; the final two-service production configuration check is `equivalent`; the web stayed on its prior healthy image `sha256:299439921295e6037a0cc01e1b1893e5bd1c2e441b740021479ca6666c1e5276` at source `3201fd249f86031292be1c47acf64e0eb8a4540b`. API `/healthz`, API `/readyz`, web `/healthz`, and web `/login` returned HTTP 200. The read-only bounded artifact inventory returned `healthy:true`, five scanned/referenced objects, zero missing or unreferenced objects, zero active operation references, and `truncated:false`. No database, web, telemetry, infrastructure, session, volume, browser, or provider action was taken.
- Point-in-time host preflight on `shaman`: 12 CPUs, 32.75 GB RAM, about 5.97 GB available, root filesystem 70% used with about 291.5 GB free, and memory PSI at 0.00 across the sampled windows. Swap was effectively full (about 8 GiB used); the short IO-PSI sample was nonzero. This was acceptable for the scoped rollout because memory pressure was absent and services remained healthy, but it is not a capacity result. The previous `docker system df` exceeded its 60-second observation bound; exact Docker image/cache/volume allocation remains unknown.
- Operator access note: the `ssh shaman` alias did not match a trusted host key, so host-key checks were not bypassed. The active workspace and `default` Docker socket were on hostname `shaman`; local read-only probes and deployment used that verified daemon. If remote SSH is needed later, establish the correct host key through a trusted channel first.
- Explicit post-release risks/follow-ups: BuildKit provenance is attached but unsigned; decide whether signed provenance is warranted before raising supply-chain assurance. Define GHCR retention/restore and disaster-recovery expectations, including a tested way to restore the private image without relying solely on the production host. Re-run Docker storage inventory during a quiet window. Monitor the shared host's nearly-full swap and IO pressure; obtain representative production traffic, explicit SLO/operating point, provider invoice, process RSS, and storage-used data before any capacity/cost claim or topology change. `insufficient_evidence` remains the capacity disposition; EVAL-011 holdout, browser layout, and real-provider behavior remain unrun. The main fast workflow's existing `actions/checkout@v4` Node deprecation notice and scheduled `ubuntu-latest` Ubuntu 26 migration (2026-10-19) are nonblocking maintenance items; the new publisher pins third-party actions by full commit SHA.
