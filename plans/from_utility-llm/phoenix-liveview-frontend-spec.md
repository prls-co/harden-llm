...
| WEB-TEST-068 | Compact workspace stats ownership | Workspace and History LiveView tests, authenticated/deployed browser canaries | `make test-fast && make test-browser` | Workspace exposes stats only inside Result/History cards, omits per-card audit shortcuts, and does not load aggregate stats; View all and aggregate totals with retry/periodic refresh remain on the audit History page. |
