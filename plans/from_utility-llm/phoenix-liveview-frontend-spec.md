...
| WEB-TEST-059 | Schema-derived response mode and repair projection | `test/harden_llm_web/live/workspace_live_test.exs` | `mix test test/harden_llm_web/live/workspace_live_test.exs` | The input widget renders neither duplicate response nor repair controls; a non-empty valid schema produces a structured request and the profile `Retries & Repair` setting controls structured repair. |
