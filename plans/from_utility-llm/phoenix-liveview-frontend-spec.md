...
- Inputs include profile, model, prompt, optional system prompt, optional JSON Schema, repair mode, web-search toggle, and supported call options defined by OpenAPI.
...
- The authenticated main UI is `/`, with `/?trace_id=...` selecting a stored result. Workspace History is the canonical list of reusable Result cards, using the numbered `page`/`limit` mode of the REST contract with direct jumps, bounded numbered navigation, page-size selection and page replacement. The legacy cursor mode remains supported for existing callers. The separate audit page and its aggregate-statistics UI are retired; `/history` and `/workspace` return 404 without compatibility redirects.
...
- Inline Details fetches `/api/v1/traces/{traceID}` and renders normalized observations in sequence in the shared foldable JSON component; available artifact links come from the same host-owned resource projection.
...
- Search fees, execution status, source links, and optional search suggestions belong inside the response fold in both Result and History. They stay hidden while the card is collapsed and expand with its existing input/output controls; model statistics remain independently visible. Folding does not change copy payloads, cached evidence, or provider execution.
...
| WEB-TEST-035 | Profile combobox/credential parity | `test/harden_llm_web/live/profiles_live_test.exs` | `mix test test/harden_llm_web/live/profiles_live_test.exs` | Searchable endpoint/model suggestions and write-only staged-key behavior remain local; canceling a staged replacement omits it from the next save request, and stored or staged secrets never render after close. |
| WEB-TEST-036 | Legacy cursor compatibility | `test/harden_llm_web/live/history_trace_test.exs`, `frontend/lib/harden_llm_web/harden_api.ex` | `mix test test/harden_llm_web/live/history_trace_test.exs test/harden_llm_web/harden_api_test.exs` | Legacy cursor clients retain their original response shape and terminal-cursor decoding while the numbered History consumer uses a separate mode. |
...
| ID | Test | Target | Command | Pass criteria |
| --- | --- | --- | --- | --- |
| WEB-TEST-044 | Server-owned widget state matrix | `test/harden_llm_web/live/profile_widget_component_test.exs`, workspace/embedding tests | `mix test test/harden_llm_web/live/profile_widget_component_test.exs` | Public LiveView events and diffs cover compact no-tabs topology, all main/nested folds, profile/reasoning/web-search/cache/retry/repair transitions, uploads, tagged parent messages, capability-aware reasoning, and independent instances. Parent fold updates preserve absent, nil, and invalid values while explicit false applies. The profile editor's numeric options and capability checkboxes retain their rendered contracts, and its combobox, datalist, and count use the same ordered model IDs after profile/catalog/custom-model changes. |
...
| WEB-TEST-052 | Utility editor defaults and help disclosures | `test/harden_llm_web/profile_defaults_test.exs`, `test/harden_llm_web/components/core_components_test.exs`, `test/harden_llm_web/live/profile_widget_component_test.exs`, `test/harden_llm_web/live/workspace_live_test.exs` | `mix test test/harden_llm_web/profile_defaults_test.exs test/harden_llm_web/components/core_components_test.exs test/harden_llm_web/live/profile_widget_component_test.exs test/harden_llm_web/live/workspace_live_test.exs` | Utility-aligned option, retry, escalation, pricing, profile/model, reasoning, and cache defaults plus rendered `?` help text remain present. Help uses independent non-submit buttons outside field labels, initially hidden inline text, associated field IDs, and client commands that toggle visibility and `aria-expanded`; these assertions do not prove browser execution or layout. Omitted, boolean, and object repair options resolve consistently during profile loading and JSON editing, and retain the same effective choice in run requests and saved profiles. |
...
| WEB-TEST-057 | Contracted schema validation and run gate | `test/harden_llm_web/live/workspace_schema_live_test.exs` | `mix test test/harden_llm_web/live/workspace_schema_live_test.exs` | Unsupported utility JSON Schema keywords are rejected and a non-empty invalid schema disables and rejects a selected structured Run; the same schema draft does not block a text Run when structured output is Off. |
...
| WEB-TEST-061 | Stats lifecycle and accounting certainty | workspace/history LiveView and component tests | `mix test test/harden_llm_web/live/workspace_live_test.exs test/harden_llm_web/live/history_trace_test.exs test/harden_llm_web/components/llm_trace_components_test.exs` | Result/provider usage and exact/partial/unknown cost retain accessible semantics. The retired aggregate-statistics UI and its polling/lifecycle tests are removed; backend aggregate accounting tests remain authoritative. |
...
| WEB-TEST-063 | Rendered canonical execution details | projection, workspace, history, and browser canary tests | `make test-fast && make test-browser` | Selected target, provider/cache/none result source, per-attempt target, result/provider accounting, artifact state render consistently. Retired v1 records are rejected at the REST boundary before projection. |
| WEB-TEST-064 | Reusable decoded JSON viewer | `test/harden_llm_web/components/json_viewer_test.exs`, trace/history component and LiveView tests | `mix test test/harden_llm_web/components/json_viewer_test.exs test/harden_llm_web/components/llm_trace_components_test.exs test/harden_llm_web/live/history_trace_test.exs` | Decoded strings retain their type, real numbers/booleans/null render distinctly, unsupported values fail explicitly, path IDs remain stable for unusual keys, empty/root values render, and structured Output/History surfaces use the same foldable component. |
| WEB-TEST-065 | Trace route identity invalidation | `test/harden_llm_web/live/workspace_live_test.exs` | `mix test test/harden_llm_web/live/workspace_live_test.exs` | A delayed response for a prior conversation route cannot replace the newly selected trace; the new route starts its own load and remains authoritative after both responses complete. |
| WEB-TEST-066 | Reusable result cards | `test/harden_llm_web/components/llm_result_components_test.exs`, workspace/rendering and authenticated browser tests | `make test-fast && make test-browser` | Current Result and workspace History use the same input/output/stats card; both text rows copy their full recorded values with emoji controls; Input/Output emoji buttons toggle both text rows and nested search evidence together with synchronized accessible state, keyboard activation, and no trailing expansion button; search fees/status/sources/suggestions are hidden when collapsed; local expansion survives unrelated patches; History cards retain independent stats/trace state and existing management actions. |
...
| WEB-TEST-069 | Canonical workspace History | `test/harden_llm_web/live/history_trace_test.exs`, authenticated/deployed browser canaries | `make test-fast && make test-browser` | Root serves the authenticated main UI; retired workspace/audit URLs return 404 without redirects. Numbered navigation replaces the displayed page, supports direct deep jumps and page-size reset, rejects duplicate requests, retries the same failed target, preserves trace URL state, and rejects stale pages after clear-all; inline trace JSON and authorized downloads replace the Domain trace dialog. |
| WEB-TEST-070 | Search diagnostics REST boundary | `test/harden_llm/llm_search_wire_test.exs`, `test/harden_llm_web/live/workspace_live_test.exs` | `make test-fast` | Optional native/Jina search metadata survives strict run, History and trace decoding and LiveView rendering; malformed metadata and unknown/absent required fields still fail. Cached evidence describes the original search, without a new provider invocation. |
...
### WEB-TEST-077: Reusable numbered pagination controls
