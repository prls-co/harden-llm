...
- Pass criteria: Convert recognized units correctly; unknown units are errors/unknown, never zero; project attribution is exact; Docker memory, RSS, sampled peak, host memory, and Docker data-root disk remain distinct; safety thresholds fail closed. Unavailable exact container measurements identify the bounded failing stage, exit status, timeout flag, and at most 256 characters of caller-redacted stderr; raw Docker output is not retained.
- Expected runtime: 5 seconds.
