...
- Source: P01 checkpoint `5f721b3` plus this phase on `feat/recovery-policy`; no production or preview deployment. Migration SHA-256: `e9c97a4c9017c34bba8d6c538b0b73b626bbdc012fe6ffad4f733c815806d0e1`.
- TEST-208: `/tmp/harden-recovery-p02-red.log` records runnable invalid-setting failures with owned services. The valid preservation fixture initially lacked the stored credential scope/type metadata required by the existing repository; that fixture was corrected. `/tmp/harden-recovery-p02-green3.log` records successful ordinary integration (approximately 26 seconds). A prior GREEN attempt exposed a PL/pgSQL name ambiguity, corrected without changing the assertions.
- EVAL-202: `/tmp/harden-recovery-p02-race.log`, `make test-integration-race`, exit 0 and accepted task, approximately 128 seconds. Final fixtures include ten profiles across two owners, two states/results, twenty invalid document cases, four concurrent migrations, repeat application and exact before/after comparisons of requests, projection values, ciphertext, sessions, observations, artifact metadata and cache rows. Migrated profiles/state/results validate against current Go/OpenAPI contracts; large numeric output is compared exactly in PostgreSQL. No race, unexpected data change, ownership violation or partial migration was observed.
- TEST-207: old bundle imports return a schemaVersion field error; old administrative catalogs fail before environment/database access. New RED evidence is `/tmp/harden-recovery-p02-import-red.log`; GREEN evidence is `/tmp/harden-recovery-p02-import-green.log` and `/tmp/harden-recovery-p02-contract-green.log`. Real resource routes also assert current server defaults and retired-bundle rejection.
- Configuration: `config/llm-profiles.example.json` is a synthetic current-format example validated by the Go catalog validator and used by the existing shared-profile Node tests (5/5). The operator-owned active file and credentials were not changed. `docs/preview-environments.md` section 6.1 defines the approved-environment cutover sequence and matching database/configuration/image restore requirement.
- TEST-201: `make test-static` exit 0 (`/tmp/harden-recovery-p02-static.log`); tracked whitespace check passed. No migration engine, converter CLI, runtime compatibility reader or replacement test task was added. The prior historical migrations remain immutable.
- Operational prerequisites: prepare all external callers/configuration, stop old writers, take the normal recoverable checkpoint, and deploy matching components only under a separately authorized cutover. Older pre-canonical histories must satisfy their existing cutover before migration 6. Local synthetic integration evidence is not deployed behavior or live-provider certification.
