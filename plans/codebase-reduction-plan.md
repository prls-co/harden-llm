...
| P05.1 final verification | In progress | FAST and targeted runtime boundaries passed. Default release failed three TEST-272/273 timing cases under load; a `--candidate-slots 1` selector passed runner-contracts but then Garage HEAD timed out while service-class integration-race overlapped CPU-class go-race. Reports and host samples are in the results ledger. Verify `make test-integration-race` alone after load returns near 8.22 before changing resource policy or retrying release. |
