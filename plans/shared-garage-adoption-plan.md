...
- Updated: 2026-09-24.
- Status: H0-H3 are complete. The deployed HLLM source/configuration revision is `a4308caf4d52326f33d29bb4070842e23992703f`; `main` is now `99780b6343d30f6fb4a0d6697cb97aec0d338cd5` after plan-only PRs #64 and #65. Local focused checks and `make test-fast` passed; hosted browser-free `make test-release` run `36064129358` passed. H4 moved the existing Garage volumes to shared-owner `main` `a572e183b448b0492f7980e833da0eabbe5926ac`; owner and HLLM services are healthy, production configuration is equivalent, and the artifact inventory is healthy. Phase H6 restarted the existing production tunnel connector; public HLLM web/API health and readiness now return HTTP 200. A signed artifact download remains unverified because the production operator login returned HTTP 403 before a session was issued. Each other consumer follows independently through its existing repository issue.
...
8. Public HTTPS requests to HLLM web/API health routes returned Cloudflare 530, error 1033. Local Caddy routes return 200, so the deployed application is healthy on-host, but external routing is not accepted. The initial inspection saw only `shaman-api-cloudflared-1`; its separate project/network means it is not the HLLM origin connector. Phase H6 records the later discovery of the dedicated stopped production connector and the focused recovery plan.
...
Status: partially complete; operational findings are recorded here and in issues #53 and `garage-shared#1`. The external public health route has been restored; signed-download verification and post-cutover monitoring remain.
...
Next: complete signed artifact verification, then observe the next normal write/trace cycle. Other consumer issues remain separate follow-up work.

## 10. Phase H6 — diagnose and restore the public Cloudflare route

Status: read-only RCA completed on 2026-09-24. It identified the stopped HLLM production-tunnel connector as the direct route failure. The existing connector was started at `2026-09-24T23:41:51Z`; public web/API health and readiness now return 200. DNS, application images/configuration, and Garage were not changed. The stop initiator and current Cloudflare-side connector inventory remain unverified.
...
| Connector logs | Earlier QUIC and Docker-DNS resolver errors were followed by successful registered connections. Immediately before exit the resolver reported connection refused, then the process received SIGTERM. | These network errors may be contributing symptoms, but the evidence does not prove they caused the shutdown. The direct, high-confidence finding is that the known connector is now stopped; Cloudflare dashboard state and any other remote connectors must be checked before claiming it is the only one. |
...
7. **Find the stop trigger and close records.** Still pending: review the owner-managed deployment/runbook and authorized host audit for why the connector received SIGTERM. Preserve the fact that the initiating actor is unknown unless evidence identifies it. Ensure the existing service remains in the owner's normal boot/start path; do not add a new monitoring subsystem. Record connector status, public probes, signed artifact result, neighboring-route results, and source/config identities in this plan and issue #53.

### Acceptance and failure behavior

- **Complete:** Cloudflare reports the correct tunnel healthy; all four public HLLM checks return 200; a signed read of a referenced artifact succeeds and matches expected content; the production descriptor stays equivalent; no other service or Garage state changed.
- **Current:** the connector is running and all four public HLLM checks return 200; the descriptor is equivalent and Garage is unchanged. Signed artifact verification remains open because the production API login returned 403, and Cloudflare dashboard status plus neighboring-route checks remain unverified.
...
- **Unexpected neighboring-route regression:** stop only the restarted HLLM connector if its current ingress is proven wrong; preserve DNS and other connectors, restore the last known-good private ingress configuration, and verify the affected production routes before retrying.

No application rebuild, Garage restart, data copy, backup/snapshot, credential rotation, browser test, or live-provider request is required for this route repair. The final gate is public HTTP and signed-artifact acceptance, not the local health check alone.
