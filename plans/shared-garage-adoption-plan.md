...
- Updated: 2026-09-24.
- Status: H0-H2 implementation is complete and merged to `main` in Harden-LLM PR #54 (`73fc735`). The implementation commit passed hosted fast and browser-free release gates; post-merge main fast T0-T2 and CodeQL checks passed. H3 source verification is complete, but private production descriptor/rollback preparation and coordinated runtime cutover remain pending. The cross-repository Analytics main gate failed before one setup suite ran; Analytics PR #14 is isolating fixture compilation in CI and its hosted checks are pending. No production runtime or data has moved.
...
Status: source verification is complete; runtime apply and final target-source preflight remain pending. Hosted fast T0-T2 and browser-free release checks passed on implementation commit `411552c` (runs `35973663949`, `35973668157`, `35974172600`); post-merge main fast T0-T2 and CodeQL checks passed after PR #54 merged as `73fc735` (runs `35994817092`, `35994816817`). Plan/status updates #55 (`3925377`) and #56 (`81c0465`) are merged; their CodeQL runs passed. The private production descriptor now allows only the planned gateway network difference and Caddy/Loki mounted-config content differences. Its check against the current live-root branch `d294a96` reports equivalent and makes no change; that is the old source, not the target. A temporary descriptor pointing to main `3925377` reported the expected gateway artifact endpoint/network differences and mount-path differences caused by the separate temp checkout; it was read-only and not applied. Final config check must run after the live bind-mounted checkout is advanced to main while affected clients are stopped.
...
- Confirm the existing `masked-recall-artifacts` bucket and scoped key against the adopted Garage at cutover. Complete a real preparation-client write and known-object read before traffic resumes.
- The production descriptor's scoped comparison allowances are prepared, but its final target check has not run against main at the original Compose path. The live checkout `d294a96` supplies bind-mounted Caddy/Loki files; advance it only after affected services stop, then run the exact production-config check before any apply.
...
- Earlier hosted Temporal-facing failures on Masked Recall head fabee32 remain unexplained; they did not recur in the verified stable-pin gates. Masked Recall PR #20's exact-head gates and main-branch gate passed. If a Temporal-facing failure recurs, retain fixture logs and assertions; this is a diagnostic follow-up, not a current gate failure.
- Bin Eval's live provider/public-ingress job still requires runner `shaman-bin-eval-live`. Port isolation is fixed in main at `395e315`; GitHub reports the runner offline despite the system service being active. Do not stop other applications to clear ports.
- Analytics main gate `36027536399` failed before its 22-case preparation suite ran: `entityStack()` exceeded the 120-second `beforeAll` hook. Its exact-head PR checks passed, so the failure is limited to that setup path, but logs do not identify the stalled operation. Analytics PR #14 (`82182f8`) builds the same tagged producer fixture binary in a named CI step and reuses it; hosted full checks are pending. Keep production cutover blocked until the PR and merged-main gate pass. If setup still times out, capture which remaining await stalled; do not raise timeouts or weaken assertions without measured evidence.
- The public exposure of the Bin Eval key is an accepted risk by explicit user choice. Preserve the exact key privately and keep its bucket-only grant; do not rotate unrelated credentials or put the value in source/docs/output.
- No backup, snapshot, compatibility alias, second daemon, browser, or provider call is part of this transition.

Next: complete Analytics PR #14's full hosted CI and merged-main gate. Restore the dedicated Bin Eval runner through authorized host service control and complete its queued live gate. Recheck private production descriptors and live container, volume, bucket, and key identities immediately before the coordinated interruption. Leave affected clients stopped if any object, permission, readiness, or signed-route check fails.
