...
- 2026-09-24 — Cross-repository validation remains open. Analytics PR #12 is at `568590b` and pins the Masked Recall candidate at `fabee32`; Masked Recall PR #20 is at `da527c8` and pins Analytics `568590b`. Current exact-head runs are pending (`36014590252`, `36014584475`, `36015017366`, `36015009635`). Earlier Masked Recall full-flow runs failed on Temporal-facing checks (workflow POST 503 and portal fixture worker-deployment context deadline); their cause is unknown. A fresh isolated local Temporal/Garage E2E passed five repetitions. Do not claim the full gate is green from that narrower result.
...
- Current hosted Temporal-facing failures have not been diagnosed. If they recur, capture Temporal/Postgres fixture logs before cleanup; do not bypass the worker-deployment or workflow API assertions.
...
Next: let the current exact-head Analytics and Masked Recall checks finish and resolve any evidenced failure without weakening the tests. Merge in dependency order after required checks pass, update cross-repository pins to stable merged main revisions, and rerun affected gates. Restore the dedicated Bin Eval runner through authorized host service control and complete its queued live job. Recheck the private production descriptor and live container, volume, bucket, and key identities immediately before the coordinated interruption. Leave affected clients stopped if any object, permission, readiness, or signed-route check fails.
