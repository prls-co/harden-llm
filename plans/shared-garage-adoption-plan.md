...
- Updated: 2026-09-24.
- Status: H0-H6 are accepted for Harden-LLM's shared-Garage transition. H6 proved public login/session behavior, an existing owner-authorized artifact download and integrity match, a temporary shared-Garage write/read/delete, current application release identity, and public HLLM health. A bounded neighboring-ingress check found unrelated route failures; evidence and follow-up boundaries are recorded in H6.C below. Other Garage consumers remain on their own open migration issues and are not claimed complete.
...
- H6 closeout identity: deployed web and gateway OCI source/revision/version are `d581942634b7197cea2d09069f73ebc89ff75bce`; their image digests are recorded in the execution log. Both service-scoped `production-config check --expected-release` commands and the whole-descriptor check returned `equivalent` with runtime verified. The shared Garage image is `dxflrs/garage:v2.3.0`, healthy.
- Retained validation: focused checks and local `make test-fast` passed; hosted browser-free `make test-release` run `36064129358` passed. H6 ran the unchanged `make test-fast` loop successfully after moving Go temporary files off the full `/tmp` tmpfs. H6's live evidence is recorded below; no source image rebuild or service recreation was needed.
- Issue: [Harden-LLM #53](https://github.com/prls-co/harden-llm/issues/53).
- H6 closeout record: [documentation-only PR #69](https://github.com/prls-co/harden-llm/pull/69); no application image or production configuration changed.
...
Status: runtime cutover and HLLM production acceptance complete on 2026-09-24. Shared Garage, local/public HLLM health, object inventory, authenticated artifact download/integrity, and temporary production write/read/cleanup passed. The separate consumer repositories remain unchanged and undeployed until their own migration issues are handled.

1. Confirmed the production checkout and owner checkout are clean, advanced `garage-shared` to `main` at `a572e183b448b0492f7980e833da0eabbe5926ac`, and advanced the production bind-mounted checkout to HLLM `main` at `a4308caf4d52326f33d29bb4070842e23992703f`. The HLLM containers were already stopped before this cutover; their initiating actor is unknown.
...
Gate: on-host Harden-LLM health, artifact inventory, and the recorded Loki query passed; Garage uses the existing volumes and publishes no host port; production HLLM source is from `main`. H6 additionally proved public login/session, authenticated artifact download and integrity, production write/read/cleanup, exact app release identity, and public web/API health/readiness. Neighboring ingress defects are separately recorded in H6.C; no consumer Garage migration is complete.

## 9. Phase H5 — close out

Status: accepted for the HLLM shared-Garage transition. Operational findings are recorded here and in issues #53 and `garage-shared#1`. Docker's existing startup and restart configuration is sufficient; a new startup service, host reboot, and Cloudflare dashboard inspection are not acceptance gates.
...
- 2026-09-24 — HLLM production containers were already stopped before this cutover; the initiating actor is unknown. `garage-shared` was advanced to main `a572e183b448b0492f7980e833da0eabbe5926ac`; Harden-LLM production source is main `a4308caf4d52326f33d29bb4070842e23992703f`.
...
- 2026-09-24 — H6.A passed: one contract-correct operator login through the public API and direct Caddy origin returned 200; each session endpoint returned 200; logout returned 200 and the revoked token returned 401. The earlier 403 was not reproduced, so no cause is claimed. No credentials or access rules changed.
- 2026-09-24 — `make test-fast` first stopped before tests because Go could not create its temporary link output on the 98%-full `/tmp` tmpfs. The unchanged command then passed all 10 tasks using a disk-backed `GOTMPDIR`; task failure was null and cleanup warnings/errors were empty. No test assertion or test scope changed.
- 2026-09-24 — H6.B authenticated as the configured production test guest after the operator account returned an empty history page. The guest already had five history items; one available artifact was selected from trace metadata. Authenticated artifact authorization returned 303, the signed download returned 200 with SHA-256 and byte count matching stored metadata, the same object URL without its signature returned 403, and logout/revocation returned 200/401. The downloaded body, trace IDs, token, and signed URL were not retained or printed.
- 2026-09-24 — H6.B storage write/read/cleanup used a temporary helper built from the existing `internal/artifacts.GarageStore` package and run in the deployed gateway's network namespace. The deployed endpoint/bucket/key/secret matched the approved private environment in memory; the effective endpoint was `http://garage-shared:3900` and region `garage`. A unique, pre-confirmed-absent JSON key passed `Put`, exact byte/hash/size `Get`, one-key `DeleteMany`, and final `Inspect` absence. The helper source/binary and key receipt were removed after confirmed cleanup; no provider call or production metadata change occurred.
- 2026-09-24 — H6.C passed: whole production descriptor and gateway/web checks with `--expected-release d581942634b7197cea2d09069f73ebc89ff75bce` all returned `equivalent`. Both app containers were healthy at that release; image digests are recorded above. Shared Garage was healthy. Public web `/healthz`, web `/login`, API `/healthz`, and API `/readyz` each returned 200.
- 2026-09-24 — The neighbor ingress pass found Grafana 200, Langfuse 200, and Allure 401 as expected under Basic Auth. Four `.prls.co` application health routes returned 502 publicly and directly from Caddy; four corresponding `.prod.agents.prls.co` names failed TLS handshake; Analytics returned 530; and the AI Knowledge alias returned a Cloudflare 307 without `Location` while direct Caddy returned the configured 308. These failures are recorded as separate ingress follow-up, not marked healthy. The HLLM Garage commit changed the artifact origin and gateway network; it did not change neighbor Caddy site definitions, Caddy network membership, or tunnel routes. No other runtime configuration was modified during H6.

Risks and follow-up checks:

- H6.A passed with the contract-correct public and origin login; the earlier 403 was not reproduced and remains unclassified. No credential or access-rule change was made.
- H6.B passed using an existing artifact owned by the configured production test guest because the operator account had no history. The signed download matched stored SHA-256/size metadata, the same unsigned object URL was denied, and the session was revoked. A unique temporary object passed Garage write/read integrity and exact-key deletion confirmation.
- H6.C passed the production descriptor, exact gateway/web release-candidate checks, shared Garage health, and public HLLM health/readiness checks. The bounded neighbor sweep found separate ingress failures, recorded below; no Garage migration or app-route change was made for them.
- H6 created no retained session or object: all acceptance sessions were revoked and the exact temporary Garage key was confirmed absent. The operator account has no trace history; the configured production test guest supplied the existing artifact used for the public download check.
- Neighbor routes to `platform.prls.co`, `masked-recall-api.prls.co`, `product-opportunity-api.prls.co`, and `synthetic-product-dataset-api.prls.co` currently return 502 from Caddy; their configured upstreams were absent from running containers/networks. The corresponding `*.prod.agents.prls.co` routes fail TLS handshake. Analytics returns 530 against an unavailable `analytics-gateway:8080` route, and `admin-aiknowledge.prls.co` returns an unexplained Cloudflare 307 without `Location`. A route-specific owner/task is needed before claiming those hosts healthy; this Garage work made no changes to their routes or upstreams.
...
Next: keep the open consumer Garage issues independently owned. A separate owner-scoped ingress task is needed to restore the failing neighbor routes listed below; it is outside this Garage transition.

## 10. Phase H6 — diagnose and restore the public Cloudflare route

Status: HLLM shared-Garage acceptance is complete. Starting the existing connector at `2026-09-24T23:41:51Z` restored public web/API health and readiness without changing DNS, application images/configuration, or Garage. The initiating cause of its SIGTERM remains unknown. H6.A-H6.C passed as recorded below. The neighboring ingress failures are not attributed to this Garage change and require a separate owner-scoped decision.
...
| Login response | The contract-correct operator request returned 200 through both public API and the direct Caddy origin. Session reads returned 200; logout returned 200; reuse of each revoked session returned 401. The earlier 403 was not reproduced. | H6.A passes without credential rotation or access-rule changes. The original 403's source remains unknown because no failing response was reproduced. |
| Artifact inventory and behavior | Existing inventory found 5/5 referenced objects. H6.B used the configured test guest's existing available artifact; the public authenticated redirect/download returned 303/200, downloaded bytes matched stored SHA-256 and size, an unsigned request to that exact object returned 403, and logout/revocation returned 200/401. The operator account had zero history items; the guest had five. | Inventory and live behavior both pass. No production trace, artifact metadata, or account data was created or deleted. |
| Garage write permission | The temporary helper ran in the deployed gateway's network namespace, after verifying its endpoint, bucket, credentials, and `prls-observability` attachment against the live container. A unique absent key passed immutable `Put`, `Get` byte/hash/size verification, `DeleteMany` of that single key, and `Inspect` absence. | H6.B passes. The helper and its source were temporary and removed; no permanent command or service was added. |
| Neighboring ingress | Grafana `/api/health`, Langfuse `/api/public/health`, and Allure `/` returned 200, 200, and the expected Caddy Basic Auth 401. Other results and source comparison are recorded in H6.C. | The HLLM Garage change did not modify the neighbor Caddy site definitions or Caddy network list. Current neighbor route failures are not attributable to the Garage transition; no pre-cutover live sample exists to claim their exact earlier status. |

### H6.A — identify and resolve the login 403

Status: passed. Public and direct-origin contract-correct login/session/logout checks all passed; the earlier 403 was not reproduced and its original source remains unknown.
...
Status: passed. The configured production test guest supplied an existing artifact; download integrity, unsigned denial, session revocation, temporary Garage write/read, and exact-key cleanup all passed. The production operator account had no history, so no production data was seeded.
...
Status: passed for HLLM. The descriptor and exact gateway/web release checks are equivalent; all four public HLLM health/login/readiness probes returned 200. Neighboring services had both healthy and failing routes; the failures below are not attributed to the Garage transition and remain separate ingress follow-up.
...
2. The bounded pass returned: Grafana `/api/health` 200; Langfuse `/api/public/health` 200; Allure `/` 401 as required by the configured Caddy Basic Auth. Requests to `platform.prls.co`, `masked-recall-api.prls.co`, `product-opportunity-api.prls.co`, and `synthetic-product-dataset-api.prls.co` health routes returned 502 both publicly and through the direct Caddy origin. Their unchanged Caddy rules target upstreams that were absent from the running containers/networks. The corresponding `*.prod.agents.prls.co` hosts failed TLS handshake before an HTTP response. `analytics.prls.co/health/ready` returned 530; the unchanged tunnel route targets `analytics-gateway:8080`, which was absent from the connector's network/runtime. `admin-aiknowledge.prls.co/` returned a Cloudflare 307 without `Location`, while the direct Caddy origin returned its configured 308 to `analytics.prls.co/dataset-reach`. These are separate ingress/upstream issues: the Garage transition commit changed the artifact origin and gateway network only; it did not change the tunnel configuration, neighbor Caddy site definitions, or Caddy network list. No neighbor routing was changed in this task. The route-owner repositories and their Garage migration issues remain independently owned; these ingress findings need a separate route-specific issue/task before those hosts can be called healthy.
...
4. Update this plan's execution record with the 403 non-reproduction evidence, test results, source/config/image identities, public endpoints, H6.B integrity and cleanup results, and neighboring-route results. H4-H6 are accepted for HLLM. Keep the separately failing ingress routes and other repositories' Garage migrations independently owned. Future normal telemetry continuity and disk checks remain routine follow-up observations.

Final HLLM acceptance: H6.A public login/session, H6.B authenticated artifact download and temporary-object write/read/cleanup, H6.C public health and production configuration, and no neighbor regression attributable to the Garage transition. The separately failing neighbor ingress routes remain explicit follow-up work and are not represented as healthy. Source tests and inventory support these checks but do not replace live boundaries. No browser or live-provider call was needed.
...
The HLLM shared-Garage transition adds no backup/snapshot feature, secret rotation, new monitoring system, alternate Garage alias, or consumer migration. Remaining work is limited to the separately owned consumer Garage migrations and the neighbor ingress defects recorded above.
