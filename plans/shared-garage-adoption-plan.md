...
- Current hosted Temporal-facing failures on the earlier Masked Recall head have not been diagnosed. Analytics now passes against that producer commit, but the Masked Recall exact-head gate remains in progress. If the failure recurs, capture Temporal/Postgres fixture logs before cleanup; do not bypass the worker-deployment or workflow API assertions.
