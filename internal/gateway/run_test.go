...
func TestRunRoute(t *testing.T) {
	...
	unconfiguredProfile := profile
	unconfiguredProfile.LLMProfile = "Unconfigured"
	unconfiguredProfile.EndpointCredentialScope = "user"
	unconfiguredDocument, err := json.Marshal(unconfiguredProfile)
	if err != nil {
		t.Fatal(err)
	}
	if err := store.SaveProfile(ctx, postgres.ProfileRecord{
		OwnerID: "owner-a", ID: unconfiguredProfile.LLMProfile, Document: unconfiguredDocument, CreatedAt: now, UpdatedAt: now,
	}, nil); err != nil {
		t.Fatal(err)
	}
	...
	// The run ID is generated by the real service; history must contain a failed
	// record even though the provider was never contacted.
	page, err := store.Runs(ctx, "owner-a", 20, nil)
	if err != nil {
		t.Fatal(err)
	}
	foundFailedUnconfigured := false
	for _, record := range page {
		if record.ProfileID == unconfiguredProfile.LLMProfile && record.Status == "failed" {
			foundFailedUnconfigured = true
			break
		}
	}
	if !foundFailedUnconfigured {
		t.Fatalf("unconfigured run history = %#v", page)
	}
