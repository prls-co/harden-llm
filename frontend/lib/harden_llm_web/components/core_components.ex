defmodule HardenLlmWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: HardenLlmWeb.Gettext
  ...
  def flash(assigns) do
    ...
    >
      <div class={[
        "flex items-start gap-3 rounded-xl border p-4 text-sm shadow-xl text-wrap",
        @kind == :info && "border-teal-200 bg-teal-50 text-teal-950",
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{
      "primary" => "bg-slate-950 text-white hover:bg-teal-800",
      nil => "border border-slate-200 bg-white text-slate-800 hover:bg-slate-50"
    }

    assigns =
      assign_new(assigns, :class, fn ->
        [
          "inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-teal-600 focus:ring-offset-2 disabled:opacity-50",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(HardenLlmWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(HardenLlmWeb.Gettext, "errors", msg, opts)
    end
  end
